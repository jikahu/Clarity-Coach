...
export default function App() {
  const [messages, setMessages] = useState<Message[]>([
    asMsg(
      "assistant",
      "Hi, this is the Clarity Coach demo. Type anything to see how it responds in real-time."
    ),
  ]);
  ...
  async function sendMessage(): Promise<void> {
    ...
    try {
      ...
      setMessages([
        ...next,
        asMsg("assistant", data.reply ?? "(No reply)"),
      ]);
    } catch (err: unknown) {
  ...
  async function clearChat(): Promise<void> {
    setMessages([
      asMsg("assistant", "Cleared! What should we tackle next?"),
    ]);
    if (!API_BASE) return;
    try {
